...
function checkPRSize(limit = BIG_PR_LIMIT) {
  if (pr.additions + pr.deletions > limit) {
    warn(
      `:eyes: Pull Request size seems relatively large (**>${limit}** modifications). If Pull Request contains multiple changes, split each into separate PR will helps faster, easier review.`
    )
  }
}
